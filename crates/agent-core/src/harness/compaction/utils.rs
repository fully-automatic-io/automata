...
pub fn extract_file_ops_from_message(message: &AgentMessage, file_ops: &mut FileOperations) {
    let AgentMessage::Assistant { content, .. } = message else { return };
    for block in content {
        let ContentBlock::ToolCall { name, arguments, .. } = block else { continue };
        let Some(path) = arguments.get("path").and_then(|p| p.as_str()) else { continue };
        match name.as_str() {
...
pub fn compute_file_lists(file_ops: &FileOperations) -> (Vec<String>, Vec<String>) {
    let modified: std::collections::HashSet<_> = file_ops.edited.iter().chain(file_ops.written.iter()).collect();
    let mut read_files: Vec<String> = file_ops.read.iter()
    ...
    for msg in messages {
        match msg {
            AgentMessage::User { content, .. } => {
                ...
                if !text.is_empty() { parts.push(format!("[User]: {}", text)); }
            }
            AgentMessage::Assistant { content, .. } => {
                    ...
                    match block {
                        ...
                        ContentBlock::ToolCall { name, arguments, .. } => {
                            let args_str = if let Some(obj) = arguments.as_object() {
                                obj.iter().map(|(k, v)| format!("{}={}", k, safe_json_stringify(v))).collect::<Vec<_>>().join(", ")
                            } else {
                ...
                if !thinking_parts.is_empty() { parts.push(format!("[Assistant thinking]: {}", thinking_parts.join("\n"))); }
                if !text_parts.is_empty() { parts.push(format!("[Assistant]: {}", text_parts.join("\n"))); }
                if !tool_calls.is_empty() { parts.push(format!("[Assistant tool calls]: {}", tool_calls.join("; "))); }
            }
